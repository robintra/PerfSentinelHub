...
# a PerfSentinelHub version before tagging it for public release.
#
# A copy of perf-sentinel's release-gate/check-lab-validation.sh, which is
# the original and carries its test suite. It is copied rather than shared
# because each repo runs its own gate from its own checkout, and a release
# must not depend on a sibling clone being present. Keep the two in step:
# a fix to the date handling or the ledger schema belongs in both.
#
