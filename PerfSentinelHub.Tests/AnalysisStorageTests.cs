...
public sealed class AnalysisStorageTests : IDisposable
{
    ...
    public async Task A_claimed_run_is_taken_once_and_the_oldest_goes_first()
    {
        ...
        await database.InsertRunAsync(Run("second", Now + 1000), cancellationToken);
        await database.InsertRunAsync(Run("first", Now), cancellationToken);
    ...
    public async Task A_run_the_service_lost_comes_back_interrupted_rather_than_replayed()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var database = await OpenAsync(cancellationToken);
        await database.InsertRunAsync(Run("queued", Now), cancellationToken);
        await database.InsertRunAsync(Run("started", Now + 1), cancellationToken);
    ...
    {
        await database.InsertRunAsync(Run(id, Now), cancellationToken);
